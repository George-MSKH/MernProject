...
const ResetPassword = () => {
  ...
  const onSubmitEmail = async (e) => {
      ...
      try {
        ...
        data.success ? toast.success(data.message) : toast.success(data.message);
    ...
    bg-linear-to-br from-blue-200 to-purple-400'>
      ...
      {!isEmailSent && 
        ...
        <form onSubmit={onSubmitEmail} className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm'>
          ...
          px-5 py-2.5 rounded-full bg-[#333A5C]'>
            <img src={assets.mail_icon} alt="" className='w-3 h3' />
            <input type="email" placeholder='Email' className='bg-transparent outline-none text-white'
            value={email} onChange={e => setEmail(e.target.value)} required/>
          </div>

          <button className='w-full py-2.5 bg-linear-to-r from-indigo-500 to-indigo-900
          text-white rounded-full mt-3 cursor-pointer'>
            Submit
          </button>
        </form>
        ...
        className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm'>
            ...
            <div className='flex justify-between mb-8' onPaste={handlePaste}>
                {Array(6).fill(0).map((_,index) => (
                    ...
                    onInput={(e) => handlerIput(e,index)}
                    onKeyDown={(e) =>handleKeyDown(e,index)}
                    />
                ))}
            </div>

            <button className='w-full py-2.5 bg-linear-to-r from-indigo-500
      ...
      {isOtpSubmitted && isEmailSent &&
        ...
        className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm'>
          ...
          <p className='text-center mb-6 text-indigo-300'>
              Enter The New Passwrd Below 
          </p>

          <div className='mb-4 flex items-center gap-3 w-full
          px-5 py-2.5 rounded-full bg-[#333A5C]'>
            <img src={assets.lock_icon} alt="" className='w-3 h3' />
